...
try{$u=ApiGuard::session();ApiGuard::permission('reports.view');$db=Connection::get();$b=(int)$u['business_id'];$branch=(int)(new BranchContextService($db))->activeBranchId($u);ApiGuard::branch($branch);$type=(string)($_GET['type']??'');$q=trim((string)($_GET['q']??''));if(mb_strlen($q)<2){echo json_encode(['ok'=>true,'items'=>[]]);exit;}$like='%'.$q.'%';
$sql=match($type){
