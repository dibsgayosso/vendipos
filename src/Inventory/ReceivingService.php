...
final class ReceivingService{
 public function __construct(private PDO $db,private PresentationService $presentations,private LotService $lots,private SerialService $serials){}
 public function receive(int $businessId,int $branchId,int $userId,array $line,?int $purchaseId=null):array{
  ...
  $qtyBase=$this->presentations->toBase($businessId,$presentationId,$qty);$lotId=null;$serialList=$line['serials']??[];
  ...
  try{
   ...
   foreach($serialList as $serial)$this->serials->receive($businessId,$branchId,$productId,(string)$serial,$lotId,$purchaseId);
   $this->db->commit();return ['qty_base'=>$qtyBase,'lot_id'=>$lotId,'serials'=>count($serialList)];
