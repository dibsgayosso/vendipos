...
final class VoidSaleService{
 ...
 public function void(int $businessId,int $branchId,int $userId,int $saleId,string $reason):void{
  ...
  try{$q=$this->db->prepare("SELECT status FROM sales WHERE id=? AND business_id=? AND branch_id=? FOR UPDATE");$q->execute([$saleId,$businessId,$branchId]);if($q->fetchColumn()!=='completed')throw new RuntimeException('La venta no puede cancelarse.');
   ...
   $pay=$this->db->prepare("SELECT COALESCE(SUM(CASE WHEN method='cash' THEN amount ELSE 0 END),0) cash_total FROM payments WHERE sale_id=?");$pay->execute([$saleId]);$cashAmount=(float)$pay->fetchColumn();if($cashAmount>0){$cs=$this->db->prepare("SELECT id FROM cash_sessions WHERE business_id=? AND branch_id=? AND user_id=? AND status='open' ORDER BY id DESC LIMIT 1 FOR UPDATE");$cs->execute([$businessId,$branchId,$userId]);$cashSessionId=$cs->fetchColumn();if(!$cashSessionId)throw new RuntimeException('Abre caja para registrar la devolución de efectivo.');$cm=$this->db->prepare("INSERT INTO cash_movements(business_id,branch_id,cash_session_id,user_id,type,amount,reference_type,reference_id,note) VALUES(?,?,?,?,'refund',?,'sale',?,?)");$cm->execute([$businessId,$branchId,$cashSessionId,$userId,$cashAmount,$saleId,trim($reason)]);}
